...
SWEP.WElements={
	["designator"]={ type="Model", model="models/saraphines/binoculars/binoculars_sniper/binoculars_sniper.mdl", bone="ValveBiped.Bip01_R_Hand", rel="", pos=Vector(0, 0, 0), angle=Angle(0, 0, 0), size=Vector(1, 1, 1), color=Color(255, 255, 255, 255), surpresslightning=false, material="", skin=0, bodygroup={} }
}
...
function SWEP:GetViewModelPosition(pos,ang)
	...
	if(not(self:GetReady()))then Downness=10 end 
...
function SWEP:PrimaryAttack()
	...
	if(self.Owner:KeyDown(IN_ATTACK2))then
		...
		elseif(SERVER)then
			...
			if(Elec>0)then self:SetElectricity(Elec-.075) end
		end
